...
fn test_run(space: &mut Space, root: Id, rules: &str) -> Option<Value> {
    ...
    space.attributes_mut(root).remove_single_named("result")
}
...
fn apply_add_attributes() {
    ...
    "), Some(Value::Int(99)));
    assert_matches!(
        space.attributes_mut(root).remove_single_named("value"),
        Some(Value::Int(23))
    );
    assert!(!space.attributes(root).has_named("value"));
    ...
    "), Some(Value::Int(23)));
    let value = space.attributes_mut(root).remove_single_named("nested").unwrap();
    ...
    assert!(space.attributes(nested).has("x", &2));
    assert!(space.attributes(nested).has("x", &3));
    let value = space.attributes_mut(root).remove_single_named("new").unwrap();
